...
sangergenotype<-function (dir=choose.dir(),link = "",wtseq="",mutseq="", revcomp=TRUE,cutoff=0.33,separator="_"){
  require("sangerseqR")
  require("stringr")
  
  # make rev complementary if required
  if (revcomp==TRUE){
  ...
  posdiff<-which(!unlist(strsplit(wt, split = ""))==unlist(strsplit(mut, split = "")))
  
  # choose target directory
  setwd(dir)
  
  # if ling given, then get files
  if (!link==""){
    dir<-paste0(dir,"\\Sequencing_",format(Sys.Date(),"%Y-%m-%d"))
    dir.create(dir)
    setwd(dir)
    download.file(link,paste0(format(Sys.Date(),"%Y-%m-%d"),".",tools::file_ext(link)))
    unzip(paste0(format(Sys.Date(),"%Y-%m-%d"),".",tools::file_ext(link)),junkpaths=TRUE)
  }
  
  # now get list of ab1 files
  files <- list.files(path=dir, pattern="*.ab1", full.names=FALSE, recursive=FALSE)
  files<-as.data.frame(files)
  files$genotype<-""
  files$files<-as.character(files$files)
  
  # get variable patterns
  tmp<-sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(files$files))
  tmp<-as.data.frame(strsplit(tmp, split = separator))
  files<-cbind(files,t(tmp[lapply(apply(fn,1,unique),length)>1,]))
  
  # now make genotyping
  print("Extracting genotypes")
  pb <- txtProgressBar(min = 0, max = length(files$files), style = 3)
  for (i in 1:length(files$files)){
    currab1<-sangerseqR::makeBaseCalls(readsangerseq(files$files[i]),cutoff)
    allels<-c(sangerseqR::primarySeq(currab1, string = TRUE),sangerseqR::secondarySeq(currab1, string = TRUE))
    ...
    if (all(str_detect(allels,wt))){
      ...
      if(all(str_detect(allels,mut))){
        ...
        if(sum(stringr::str_detect(allels,wt),stringr::str_detect(allels,mut))==2){
